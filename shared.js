...
const discWidget = document.getElementById('discordWidget');
function applyTheme(t) {
  html.setAttribute('data-theme', t);
  try { localStorage.setItem('tame-theme', t); } catch (e) { }
  if (discWidget) discWidget.src = 'https://discord.com/widget?id=1012876825573204048&theme=' + (t === 'dark' ? 'dark' : 'light');
}
const savedTheme = (() => { try { return localStorage.getItem('tame-theme'); } catch (e) { return null; } })();
applyTheme(savedTheme || (window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light'));
if (themeBtn) themeBtn.addEventListener('click', () => applyTheme(html.getAttribute('data-theme') === 'light' ? 'dark' : 'light'));
...
if (hamburgerBtn && mobileMenu) {
  ...
  if (mmOverlay) mmOverlay.addEventListener('click', () => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Magnetic Buttons
  document.querySelectorAll('.ss-btn, .port-btn, .soc, .fsoc, .mctt-copy-btn').forEach(btn => {
